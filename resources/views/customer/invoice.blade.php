        ...
        <div class="w-full px-4 sm:px-8 lg:px-12 2xl:px-16 space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white/95 backdrop-blur-xl p-4 sm:p-5 rounded-3xl border border-[#DFC387] shadow-sm">
                <div class="flex items-center gap-3.5">
                    <a href="{{ route('dashboard') }}" class="p-2.5 rounded-2xl bg-[#FAF2DE] hover:bg-[#F3DFAD] border border-[#DFC387] text-[#7A5818] transition-colors shrink-0" title="Back to Home">
                    ...
                    <div>
                        <div class="flex items-center gap-2 flex-wrap">
                            <h1 class="font-serif font-black text-xl sm:text-2xl text-[#1F170D]">Invoice &amp; Digital E-Ticket</h1>
                            <span :class="ticket && (ticket.status === 'PAID' || ticket.status === 'CONFIRMED' || ticket.status === 'CHECKED_IN') ? 'bg-emerald-100 text-emerald-800 border-emerald-300' : (ticket && (ticket.status === 'EXPIRED' || ticket.status === 'CANCELLED' || ticket.status === 'REFUNDED') ? 'bg-rose-100 text-rose-800 border-rose-300' : 'bg-amber-100 text-amber-800 border-amber-300')"
                                  ...
                                  x-text="ticket ? ticket.status : 'LOADING...'">
                            </span>
                        </div>
                        <p class="text-xs text-[#7A643E] mt-0.5">Present this digital E-Ticket QR Code at the venue turnstile gate for instant court access</p>
                    </div>
                </div>
            ...
            <!-- Auto-Polling Banner when Status is Pending -->
            <div x-show="isPolling" 
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div class="text-xs font-black uppercase tracking-wider">Awaiting Midtrans Payment Confirmation...</div>
            ...
            <div x-show="isLoading" class="py-16 text-center space-y-3">
                ...
                <div class="text-xs font-bold text-[#8C6418]">Loading e-ticket data from database...</div>
            </div>
