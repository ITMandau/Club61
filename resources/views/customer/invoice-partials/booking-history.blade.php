...
<div class="bg-white/95 backdrop-blur-xl rounded-3xl border border-[#DFC387] shadow-[0_12px_35px_rgba(160,120,30,0.15)] p-4 sm:p-6 space-y-4">
    ...
    <div class="space-y-2" x-show="pastBookings.length > 0">
        <!-- Row 1: Search Bar for Court Name & Booking Code -->
        <div class="relative w-full">
            ...
            <input type="text" 
                   x-model="searchQuery" 
                   @input="currentPage = 1"
                   placeholder="Search court name (Court 1, 2) / code..." 
            ...
            <button type="button" 
                    ...
                    class="absolute inset-y-0 right-0 pr-3 flex items-center text-[#8C7A58] hover:text-rose-600 transition-colors cursor-pointer">
        ...
        <div class="flex items-center justify-between gap-2 text-xs">
            <div class="flex items-center gap-2 flex-1 min-w-0">
                ...
                <input type="date" 
                       ...
                       title="Filter reservation date"
                       class="flex-1 min-w-0 px-2.5 py-1.5 rounded-xl border border-[#DFC387] text-xs focus:ring-1 focus:ring-[#D4AF37] focus:outline-none bg-[#FAF8F2] text-[#1F170D] shadow-sm cursor-pointer">
            </div>

            <button type="button" 
                    x-show="searchQuery || searchDate" 
                    @click="resetFilters()" 
                    class="px-2.5 py-1.5 rounded-xl border border-rose-200 bg-rose-50 text-rose-600 hover:bg-rose-100 text-[11px] font-bold transition-all shrink-0 flex items-center gap-1 shadow-sm cursor-pointer">
                <span>Reset</span>
    ...
    <div x-show="totalPages > 1 && paginatedBookings.length > 0" 
        ...
        <button type="button" 
                ...
                class="px-3 py-1.5 rounded-xl border border-[#DFC387] text-[#7A5818] font-bold text-[11px] hover:bg-[#FAF2DE] disabled:opacity-40 disabled:cursor-not-allowed transition-all flex items-center gap-1 bg-white cursor-pointer">
...
<div class="p-4 sm:p-5 rounded-3xl bg-gradient-to-r from-[#FAF2DE] via-[#F5E6BE] to-[#FAF2DE] border border-[#DFC387] flex items-center justify-between gap-4">
    <div>
        <h4 class="font-serif font-black text-sm text-[#1F170D]">Need Assistance with Check-In?</h4>
        <p class="text-xs text-[#7A643E]">Our concierge frontdesk is ready to assist you at the turnstile gate.</p>
    </div>
