...
<div x-show="showCancelModal" 
    ...
    <div class="w-full max-w-md bg-white rounded-3xl border-2 border-[#D4AF37] shadow-2xl p-6 sm:p-7 space-y-5 animate-scaleIn text-center relative">
        ...
        <!-- Content -->
        <div class="space-y-2">
            <h3 class="font-serif font-black text-xl text-[#1F170D]">Cancel Court Booking?</h3>
            <p class="text-xs text-[#7A643E] leading-relaxed">
                The locked court slots will be immediately released back to the public schedule for other players. You can select a new schedule after cancellation.
            </p>
        </div>
            ...
            <div class="p-3.5 rounded-2xl bg-[#FAF8F2] border border-[#E8DCC0] text-left text-xs space-y-2">
                ...
                <template x-if="ticket && ticket.order_bookings && ticket.order_bookings.length > 1">
                    <div class="flex justify-between items-center text-xs pt-1 border-t border-[#DFC387]/40">
                        ...
                        <span class="font-bold text-[#8C6418]" x-text="ticket.order_bookings.length + ' Linked Court Session' + (ticket.order_bookings.length > 1 ? 's' : '')"></span>
                    </div>
                </template>
                <div class="flex justify-between items-center text-xs pt-1 border-t border-[#DFC387]/40">
                    <span class="text-[#7A643E]">Booking Code:</span>
            ...
            <button type="button" 
                ...
                <template x-if="!isCancellingBooking">
                    <span class="flex items-center gap-1.5" style="color: #FFFFFF;">
                        ...
                        <span style="color: #FFFFFF;">Yes, Cancel Booking</span>
                    </span>
                </template>
                <template x-if="isCancellingBooking">
                    <span class="flex items-center gap-2" style="color: #FFFFFF;">
                        ...
                        <span style="color: #FFFFFF;">Releasing Court Slots...</span>
                    </span>
                </template>
            </button>
