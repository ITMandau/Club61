...
<div class="bg-white/95 backdrop-blur-xl rounded-3xl border-2 border-[#D4AF37] shadow-[0_20px_50px_rgba(160,120,30,0.22)] overflow-hidden">
    ...
    <div class="p-4 sm:p-6 lg:p-8 text-center flex flex-col gap-5 items-stretch">
        ...
        <template x-if="currentTicket.status !== 'PAID' && currentTicket.status !== 'CHECKED_IN'">
            <div class="max-w-md mx-auto p-5 sm:p-6 rounded-3xl border-2 border-dashed border-amber-300 bg-amber-50/70 text-center space-y-4">
                ...
                <div class="p-3.5 rounded-2xl bg-white border border-[#DFC387] flex items-center justify-between text-left shadow-sm">
                    <div class="flex items-center gap-3">
                        <span class="w-8 h-8 rounded-lg bg-[#FAF8F2] border border-[#DFC387] flex items-center justify-center font-bold text-[10px] text-[#8C6418] font-mono shrink-0" x-text="selectedMethod.badge"></span>
                        <div>
                ...
                <div x-show="selectedMethod.code === 'CASH' || isCashNotice || (currentTicket.order && currentTicket.order.payment_method === 'CASH')" 
                    ...
                    <p class="text-[11px] text-[#5C410F] leading-snug">
                        Tunjukkan Kode Booking <strong class="font-mono text-[#1F170D]" x-text="'#' + (currentTicket.booking_code || currentTicket.id.substring(0, 8))"></strong> kepada kasir frontdesk venue Club61 untuk melunasi reservasi secara tunai dan mengaktifkan e-tiket Anda.
                    </p>
                </div>
